...
def save_test_result(opt):
    ...
    for gt_file in gt_list:
        ...
        # 좌표거리 계산
        results_df[__coo_dist] = coo_dist(new_coo_df)
        # 영역크기차이 계산
        results_df[__sub_area] = subtract_area(new_coo_df)
        # gt데이터의 개수구하기
        ...
        results_df.loc[gt_cl_mask, [__gt_class, __gt_x, __gt_y, __gt_w, __gt_h]] = -1

        pred_cl_mask = results_df[__pred_class].isnull()
        # results_df.loc[pred_cl_mask, [__pred_ogj_cnt]] = 0
        results_df.loc[pred_cl_mask, [__pred_class, __pred_x, __pred_y, __pred_w, __pred_h, __pred_conf]] = -1

        # 행의 개수가 1보다 클 경우,  중심점의 차이~클래스 일치도 까지를 0으로 채우기
        if len(results_df) > 1:
...
def coo_dist(coo_df):
    ...
    new_coo_df[__coo_dist] = np.sqrt(x ** 2 + y ** 2)
...
def subtract_area(coo_df):
    ...
    new_coo_df[__sub_area] = abs(gt_area - pred_area)
...
def get_excel(excel_root, file_name):
    if os.path.exists(f'{excel_root}/{file_name}') :
        ...
        index = [__row_num, __model_name, __gt_class, __gt_x, __gt_y, __gt_w, __gt_h,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gt_root', type=str, default='/Users/lfin/Documents/Lfin/SM/data/test/safety_test_20220203_416size.yolov5pytorch', help='정답txt파일이 저장된 위치')
    parser.add_argument('--pred_root', type=str, default='/Users/lfin/Documents/Lfin/SM/data/detect/10_detect_results/3090_20220211_kaggle2+crop/labels', help='예측txt파일이 저장된 위치')
    parser.add_argument('--excel_root', type=str, default='/Users/lfin/Documents/Lfin/SM/data/detect/00_excel',  help="excel파일을 저장할 위치")
    # parser.add_argument('--model_name', type=str, default='3090_20220207_kaggle2',  help="모델명")
